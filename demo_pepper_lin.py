...
class socket_connection():
    ...
    def set_head(self, pitch, yaw):
        self.s.sendall(bytes("shea {:0.2f} {:0.2f}".format(pitch, yaw).encode()))
...
def prediction(transformations, model, frame):
    ...
    if faces is not None:
        for box, landmarks, score in faces:
            ...
            pitch_predicted = softmax(
                gaze_pitch)  # this is not pitch but its actually yaw. We are leaving it as it is for coherence with other analysis
            yaw_predicted = softmax(
                gaze_yaw)  # # this is not yaw but its actually pitch. We are leaving it as it is for coherence with other analysis

            # Get continuous predictions in degrees.
            pitch_predicted = torch.sum(pitch_predicted.data[0] * idx_tensor) * 4 - 180
            yaw_predicted = torch.sum(
                yaw_predicted.data[0] * idx_tensor) * 4 - 180  # compensation (remembner that this ios actually pitch)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #connect.say('test!')
        if pitch != None:
            connect.set_head(pitch_predicted[-1], -yaw_predicted[-1])
            time.sleep(2)
            flag=1
        ...
        # print("Restore")
        # connect.idle()
        # connect.adjust_head(-pitch_predicted[-1] + pitch_predicted[-2], yaw_predicted[-1] - yaw_predicted[-2])
        # connect.adjust_head(pitch_predicted[-1], -yaw_predicted[-1])
        # time.sleep(3)

        # print(pitch_predicted_full[-5:])
        # if pitch_predicted_full[-5:] == [None, None, None, None, None]:
        #     print("Restore")
        #     connect.set_head(-0.1, 0)
        #     time.sleep(3)
